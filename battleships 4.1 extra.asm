...
;AI variables
Last_Hit_Part1 dw -1 ;remembers the offset of the last ship part hit, resets to -1 when ship is down
Known_Direction1 db -1 ;remebers the direction of the ship, -1 means the direction is unknown, 0 means up, 1 down, 2 left, 3 right
Last_Hit_Part2 dw -1 ;remembers the offset of the last ship part hit, resets to -1 when ship is down
Known_Direction2 db -1 ;remebers the direction of the ship, -1 means the direction is unknown, 0 means up, 1 down, 2 left, 3 right
...
;input: computer1's: board, guessing board, left ship array
;		computer2's: board, guessing board, left ship array
proc no_player_Match
	push bp
	mov bp, sp
	push ax
	push dx
...
turn_cycle1:
	...
	push ax
	push offset Last_Hit_Part1
	push offset Known_Direction1
	call computer_turn
	cmp al, 1
	je win2
	call Wait_for_key_press
	...
	mov ax, [bp+10] ;playerLeftShip
	push ax
	push offset Last_Hit_Part2
	push offset Known_Direction2
	call computer_turn
	cmp al, 1
	je lose1
	call Wait_for_key_press
